...
def search_for_urls(company_description: str, firecrawl_api_key: str, num_links: int) -> List[str]:
    ...
    payload = {
        ...
        "limit": num_links,
        "lang": "en",
        "location": "India",  # Adjusted to India for IFIM College context
        "timeout": 60000,
    }
...
def extract_user_info_from_urls(urls: List[str], firecrawl_api_key: str) -> List[dict]:
    ...
    try:
        for url in urls:
            st.write(f"Processing URL: {url}")  # Debugging output
            response = firecrawl_app.extract(
                [url],
                {
                    'prompt': 'Extract all user information from Quora posts, including username, bio (if available), post type (question/answer), timestamp, upvotes, and any links. Focus on users asking or answering questions related to management education or IFIM College. Return empty fields if data is unavailable.',
                    'schema': QuoraPageSchema.model_json_schema(),
                }
            )
            
            st.write(f"Raw Firecrawl Response for {url}: {response}")  # Debugging output
            if response.get('success') and response.get('status') == 'completed':
                ...
                if interactions:
                    ...
                    st.warning(f"No interactions found for URL: {url}")
            else:
